...
export async function newPayment(cardId: number, cardPassword: string, businessId: number, amount: number) {
...
export async function insertPaymentData(cardId: number, businessId: number, amount: number) {
    const paymentData = {
        cardId,
        businessId,
        amount,
...
export async function cardHasEnoughBalance(card: cardRepository.Card, amount: number) {
